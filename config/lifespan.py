...
from config.settings import scheduler, settings
...
async def lifespan(app: FastAPI) -> None:
    ...
    db_client: AsyncIOMotorClient = await get_mongo_db_client(mongo_db_url=settings.MONGO.URL_DOCKER)
    database: AsyncIOMotorDatabase = await get_mongo_db(
        db_client=db_client, db_name=settings.MONGO.REAL_DB_NAME
    )
