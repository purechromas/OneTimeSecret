class TestSecretAPI:
    ...
    async def test_get_secret_404_without_password(self, async_client):
        ...
        response_404 = await async_client.post(
            f"secret/review_secret/{verification_number_404}/"
        )
    ...
    async def test_get_secret_200_without_password(self, async_client):
        ...
        response_401 = await async_client.post(
            f"secret/review_secret/{verification_number_200}/"
        )
    ...
    async def test_get_secret_200_with_correct_password_and_404_removed(
        self, async_client
    ):
        response = await async_client.post("secret/create_secret/", json=self.data_body)
        verification_number_200 = response.json()["data"]["verification_number"]
        ...
        assert response_200.json() == {
            "data": {"secret_msg": "YourSecretMessage"},
            "detail": 200,
        }
